"""
Binary Particle Swarm Optimization (BPSO) for Battery Management System (BMS) Feature Selection.
"""
...
def eval_bpso(particle, X_train, X_test, y_train, y_test):
...
def run_bpso(num_particles=15, max_iterations=30):
    X, y = load_data()
    ...
    print("--- Running Binary Particle Swarm Optimization (BPSO) ---")
    start_time = time.time()

    for i in range(num_particles):
        f, acc, f1, num_sel, mdr = eval_bpso(pos[i], X_train, X_test, y_train, y_test)
        p_best_fit[i] = f
        if f < g_best_fit:
    ...
    print(f"BPSO Complete in {time.time()-start_time:.2f}s")
